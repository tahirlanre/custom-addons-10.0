            ...
            for batch_line in batch.batch_line_ids:
                ...
                if (batch_line.account_id or batch_line.partner_id) and not batch_line.journal_entry_ids.ids:
                    if batch_line.payment > 0.0:
                        amount = batch_line.payment * (batch_line.payment_type in ('ap', 'gl','ar') and 1 or -1)
                    elif batch_line.deposit > 0.0:
                        amount = batch_line.deposit * (batch_line.payment_type in ('ap', 'gl','ar') and -1 or 1)
...
class account_cashbook_batch_line(models.Model):
    ...
    description = fields.Char(string='Description', default = 'Payment on Account',required=True)
    payment_type = fields.Selection([('ar', 'Account Receivable'), ('ap', 'Account Payable'),('gl', 'General Ledger')], string='Payment Type', default='ar')

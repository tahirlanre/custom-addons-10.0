...
class InventorySalesAnalysisXlsx(ReportXlsx):
    ...
    def __init__(self, name, table, rml=False, parser=False, header=True,
                 store=False):
        super(InventorySalesAnalysisXlsx, self).__init__(
    ...
    def generate_xlsx_report(self, workbook, data, objects):
        ...
        if report.group_by_product:
            self.columns = self._get_product_report_columns()
        
        if report.group_by_partner:
            self.columns = self._get_partner_report_columns()
        
        if report.group_by_sales_rep:
            self.columns = self._get_salesrep_report_columns()
    ...
    def _generate_report_content(self, workbook, report):
        ...
        if report.group_by_sales_rep:
            for sales_rep in report.sales_rep_ids:
                self.write_line(sales_rep)
...
InventorySalesAnalysisXlsx('report.inventory_sales_analysis.inventory_sales_analysis_xlsx','report_inventory_sales_analysis_qweb',parser=report_sxw.rml_parse)